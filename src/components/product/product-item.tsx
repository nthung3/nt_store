...
import { addToCart, openCart } from '@/lib/features/cart/cart-slice';
...
export default function ProductItems({ data }: ProductItemsProps): JSX.Element {
        ...
        >
            <div>
                ...
                <div className="relative block h-full">
                    {data._id && (
                        <Link href={`/restaurant/${data._id}`} className="absolute top-0 left-0 w-full h-full" />
                    )}
                    <img src={data.img} alt={data.name} className="object-cover w-full h-64 bg-gray-100 rounded-3xl" />
                </div>
            </div>
